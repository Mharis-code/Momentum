...
```
Skill(skill="frontend-design", args="Generate a production-grade, visually distinctive funnel strategy reference document in HTML. Design direction: choose a modern growth/marketing aesthetic. Brand color: PRIMARY=[resolved color hex]. Required sections: Lead Magnet (selected option in a callout card with name, format, why it works; dismissed options below in muted text), Landing Page Copy (headline as large text, subheadline, 3 benefit bullets, social proof testimonial card, CTA button text, trust line), Welcome Sequence (5 email cards, one per day, subject line in bold and body text), Platform Bio CTAs (card per platform with character count badge), Weekly Posting Rhythm (4-week Mon/Wed/Fri table as HTML table), Booking Page Brief (spec card with all sections). Typography: Google Fonts @import — one distinctive heading font, no Inter, Roboto, or system fonts. CSS animations: subtle fade-in on sections, hover states on cards. Self-contained: no external CSS or JS — Google Fonts @import is the only allowed external dependency. Insert <!-- SECTION-NAME --> HTML comments as injection points for each section. Return the complete HTML shell with all CSS and structure — no bracketed placeholder text.")
```
After frontend-design returns the shell, fill in every `<!-- SECTION-NAME -->` injection point with the real funnel content.
Save as `[output_folder]/funnel-strategy.html`.

**If Momentum tab mode (IS_MOMENTUM=true):**
Generate the tab content as an HTML fragment (not a full `<!DOCTYPE html>` document). Use the CSS variables already defined in the parent `90-day-plan.html`: `--primary`, `--accent`, `--text`, `--text-muted`, `--bg`, `--border`, `--primary-light`. Scope any additional styles inside a `<style>` block with selectors prefixed by `#tab-funnel` to avoid bleeding into other tabs. Render all 6 sections listed above.
...
3. Find `<!-- TAB-CONTENT-END -->` and insert before it the fragment wrapped in:
   `<div class="tab-content" id="tab-funnel">[rendered HTML fragment]</div>`
...
**If standalone mode (IS_MOMENTUM=false):**
Call the frontend-design skill to generate the landing page:
```
Skill(skill="frontend-design", args="Generate a production-grade, conversion-optimized landing page for a lead magnet. This must look like a real deployable product page — not a document. Required sections: (1) full-width hero block with large headline and subheadline; (2) benefit bullets with styled check marks; (3) social proof testimonial card; (4) large CTA button (full width on mobile); (5) trust line below the CTA. Brand color: PRIMARY=[resolved color hex]. Typography: Google Fonts @import — one distinctive, high-converting heading font, no Inter, Roboto, or system fonts. CSS: mobile-responsive, smooth scroll, CTA hover state. Self-contained: no external CSS or JS — Google Fonts @import only. Insert injection comments: <!-- HEADLINE -->, <!-- SUBHEADLINE -->, <!-- BULLET-1 -->, <!-- BULLET-2 -->, <!-- BULLET-3 -->, <!-- TESTIMONIAL -->, <!-- CTA-TEXT -->, <!-- TRUST-LINE -->. Return the complete HTML shell.")
```
Fill in all injection points with content from Step 3. Save as `[output_folder]/landing-page.html`.
Tell the user the paths for both `funnel-strategy.html` and `landing-page.html`.

**If Momentum tab mode (IS_MOMENTUM=true):**
Generate the landing page HTML with scoped styles — prefix every CSS class with `#tab-landing-page` to prevent style bleed into other tabs. The hero block must use `var(--primary)` for the background color. Fill in all content from Step 3.

Then update 90-day-plan.html:
1. Find `<!-- TAB-BUTTONS-END -->` and insert before it:
   `<button class="tab-btn" data-tab="landing-page">Landing Page</button>`
2. Find `<!-- TAB-CONTENT-END -->` and insert before it the scoped landing page wrapped in:
   `<div class="tab-content" id="tab-landing-page" style="padding:0;">[scoped landing page HTML]</div>`
3. If tabs already exist, replace rather than insert.
4. Save the modified `90-day-plan.html`.
5. Tell the user: "Funnel Strategy and Landing Page tabs added to 90-day-plan.html."
